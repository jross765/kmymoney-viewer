...
import org.kmymoney.viewer.widgets.TransactionSum;
import org.kmymoney.viewer.widgets.TransactionSum.SUMMATIONTYPE;
...
public class TaxReportPanel extends JPanel {
	 ...
	 */
	public TaxReportPanel(final KMyMoneyFile books) {
		if (books != null) {
			initializeUI(books);
		}
	}
	 ...
	 */
	private void initializeUI(final KMyMoneyFile books) {
	 ...
	 */
	private TransactionSum createSum(final KMyMoneyFile books, final Properties prop, final int index) throws InvalidKMMIDException, NoEntryFoundException, TooManyEntriesFoundException {
		SUMMATIONTYPE type = SUMMATIONTYPE.getByName(prop.getProperty("sum." + index + ".type"));
		String name = prop.getProperty("sum." + index + ".name");
		Set<KMyMoneyAccount> target = getAccountsByProperty(books, prop, "sum." + index + ".target");
		Set<KMyMoneyAccount> source = getAccountsByProperty(books, prop, "sum." + index + ".source");
		TransactionSum sum = new TransactionSum(books, source, target, type, name, getMinDate(), getMaxDate());
		mySums.add(sum);
		return sum;
	}

	private Set<KMyMoneyAccount> getAccountsByProperty(final KMyMoneyFile aBooks,
			final Properties props, final String prefix) throws InvalidKMMIDException, NoEntryFoundException, TooManyEntriesFoundException {

		Set<KMyMoneyAccount> retval = new HashSet<KMyMoneyAccount>();
		for (int i = 0; props.containsKey(prefix + "." + i); i++) {
			String idOrName = props.getProperty(prefix + "." + i);
			KMyMoneyAccount account = aBooks.getAccountByIDorName(new KMMComplAcctID(idOrName), idOrName);
			if (account == null) {
